...
function SidebarToggle({ isSidebarOpen, setIsSidebarOpen }) {
  ...
  return (
    <button
      ...
      className={`fixed bottom-8 group cursor-pointer flex space-x-3 items-center mt-4 px-6 transition
           bg-mainPurple rounded-r-full hover:bg-opacity-10 bg-opacity-0 dark:hover:bg-white dark:hover:text-mainPurple
          ${
            isSidebarOpen
              ? 'text-mediumGrey'
              : 'h-12 w-16 bg-opacity-100 hover:bg-mainPurpleHover text-white'
          }`}
    >
      {isSidebarOpen ? (
        <>
